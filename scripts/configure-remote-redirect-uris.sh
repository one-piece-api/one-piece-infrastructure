#!/usr/bin/env bash
# Aggiunge l'IP pubblico riservato ai redirect URI del client "onepiece-proxy",
# solo nell'ambiente "remote" - keycloak/realm-onepiece.json resta fisso su
# "localhost" (locale/CI, unica source of truth per quegli ambienti), e
# "--import-realm" non aggiorna un client già esistente in un realm già
# importato (bug noto, vedi il commento in apply-realm-configmap.sh), quindi
# il redirect URI per l'ambiente remoto va corretto via Admin API dopo ogni
# sync, non nel JSON.
#
...
kubectl exec -n auth statefulset/keycloak -- bash -c '
  ...
  client_id="$(/opt/keycloak/bin/kcadm.sh get clients -r onepiece -q clientId=onepiece-proxy --fields id --format csv --noquotes)"
  /opt/keycloak/bin/kcadm.sh update "clients/$client_id" -r onepiece \
...
echo "[configure-remote-redirect-uris] redirect URI del client 'onepiece-proxy' aggiornati per $OCI_LB_IP."
